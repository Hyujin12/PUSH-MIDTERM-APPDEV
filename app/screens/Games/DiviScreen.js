...
const DiviScreen = () => {
  ...
  const generateQuestion = () => {
    ...
    let incorrectAnswer;
    const generateUniqueIncorrectAnswer = () => {
      incorrectAnswer = correctAnswerValue + (Math.floor(Math.random() * 3) + 1); // ensure variety in wrong answer
      return incorrectAnswer !== correctAnswerValue;
    };
    
    // Ensure we get a unique incorrect answer
    while (!generateUniqueIncorrectAnswer()) {}
  ...
  const showScoreAlert = () => {
    // Ensure final score is shown correctly
    setTimeout(() => {
      Alert.alert(
        "Time's up!",
    ...
    }, 100); // Small delay to ensure score is fetched
  };
